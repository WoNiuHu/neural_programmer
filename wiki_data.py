...
def correct_unicode(string):
  ...
  string = re.sub(b"\xc2\xa0", b" ", string.encode()).decode().strip()
  string = re.sub(b"\xe2\x80\x93", b"-", string.encode()).decode().strip()
  #string = re.sub(ur'[\u0300-\u036F]', "", string)
  # string = re.sub(b"â€š", ",", string)
  # string = re.sub(b"â€¦", "...", string)
  # #string = re.sub("[Â·ãƒ»]", ".", string)
  # string = re.sub(b"Ë†", "^", string)
  # string = re.sub(b"Ëœ", "~", string)
  # string = re.sub(b"â€¹", "<", string)
  # string = re.sub(b"â€º", ">", string)
